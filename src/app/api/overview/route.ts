...
import { z } from "zod";
import { exampleCards } from "@/app/data/mock";
...
const Body = z.object({
  transactions: z.array(Tx).default([]),
  currency: z.string().optional(),
});

export async function GET() {
  return NextResponse.json(exampleCards);
}
...
export async function POST(req: Request) {
  ...
  const income = transactions
    .filter((t) => t.type === "income" && t.currency === ccy)
    .reduce((s, t) => s + t.amount, 0);
  const outcome = transactions
    .filter((t) => t.type === "outcome" && t.currency === ccy)
    .reduce((s, t) => s + t.amount, 0);
  ...
  if (income > 0) {
    if (rate >= 0.2)
      advice =
        "Solid month. Savings rate ≥20%. Keep it up and watch fixed costs.";
    else if (rate >= 0.1)
      advice =
        "Decent. Push discretionary spend down ~5–10% to hit 20% savings.";
    else
      advice =
        "Warning: spending high. Audit top 3 categories and set weekly caps.";
  }
